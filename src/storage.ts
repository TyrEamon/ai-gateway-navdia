...
export async function getProviders(env: Env): Promise<Provider[]> {
  const data = await env.AIAGENT_PROXY.get(KV_KEYS.PROVIDERS)
...
export async function setProviders(env: Env, providers: Provider[]): Promise<void> {
  await env.AIAGENT_PROXY.put(KV_KEYS.PROVIDERS, JSON.stringify(providers))
}
...
export async function createSession(env: Env, username: string, ttlSeconds: number): Promise<string> {
  ...
  await env.AIAGENT_PROXY.put(KV_KEYS.SESSION_PREFIX + sessionId, JSON.stringify(session), {
...
export async function getSession(env: Env, sessionId: string): Promise<Session | null> {
  const data = await env.AIAGENT_PROXY.get(KV_KEYS.SESSION_PREFIX + sessionId)
  if (!data) return null
...
export async function deleteSession(env: Env, sessionId: string): Promise<void> {
  await env.AIAGENT_PROXY.delete(KV_KEYS.SESSION_PREFIX + sessionId)
}

// ===== 转发 Key =====

export async function getProxyKeys(env: Env): Promise<ProxyKey[]> {
  const data = await env.AIAGENT_PROXY.get(KV_KEYS.PROXY_KEYS)
  return data ? JSON.parse(data) : []
}

export async function setProxyKeys(env: Env, keys: ProxyKey[]): Promise<void> {
  await env.AIAGENT_PROXY.put(KV_KEYS.PROXY_KEYS, JSON.stringify(keys))
}
